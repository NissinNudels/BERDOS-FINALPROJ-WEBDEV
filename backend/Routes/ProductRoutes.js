...
productRoute.get("/", asyncHandler(async (req, res) => {
  const pageSize = 6; //no. of prod in each page
...
productRoute.get("/all", protect, admin, asyncHandler(async (req, res, next) => {
  try {
...
productRoute.post( "/:id/review", protect, asyncHandler(async (req, res) => {
      ...
      if (product) {
        ...
        if (alreadyReviewed) {
          res.status(400);
          throw new Error("Product already given Review");
        }
        const review = {
          name: req.user.name,
...
// DELETE PRODUCT
productRoute.delete("/:id", protect, admin, asyncHandler(async (req, res, next) => {
  const product = await Product.findById(req.params.id);
  if (product) {
